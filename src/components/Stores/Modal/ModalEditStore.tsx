...
import { getById, updateStore } from "@/src/services/store.service";
import { toast } from "react-toastify";

export default function ModalEditStore({ isOpen, onClose, id, reloadStores, classifications, segments, activity }: {
    isOpen: boolean,
    onClose: () => void,
    id: number,
    reloadStores: () => void,
    ...
    useEffect(() => {
        ...
        const filtered = segments.filter(
            s => s.classification_id === classificationId
        );
    ...
    const handleSubmit = async () => {
        ...
        try {
            setIsLoading(true);
            const created = await updateStore(id, {
                name: name,
            ...
            if (created == true) {
                ...
                reloadStores();
                Swal.fire({
                    title: 'Sucesso!', icon: "success", text: "Loja alterada com sucesso!", confirmButtonText: "Boa!"
                })
            }
        }
        catch (err: any) {
    ...
    return (
        <Modal title="Editar Loja" width={30} widthMobile={95} isOpen={isOpen} onClose={onClose}>
            {isLoading ?
                (
                    <>
